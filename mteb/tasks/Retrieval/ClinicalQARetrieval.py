...
class ClinicalQARetrieval(AbsTaskRetrieval):
    ...
    def load_data(self, **kwargs):
        ...
        self.queries = {self._EVAL_SPLIT: {str(i): row['question'] for i, row in enumerate(data) if row['task'] == 'Paraphrasing'}}
        self.corpus = {self._EVAL_SPLIT: {str(row['patient_id']): {'text': row['answer']} for row in data if row['task'] == 'Paraphrasing'}}
        self.relevant_docs = {self._EVAL_SPLIT: {str(i): {str(row['patient_id']): 1} for i, row in enumerate(data) if row['task'] == 'Paraphrasing'}}
